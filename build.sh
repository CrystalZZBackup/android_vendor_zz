...
if [ $ARCH = "64" ]; then
  # Get build version
  MAJOR=$(cat $DIR/vendor/zz/config/version.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
  MINOR=$(cat $DIR/vendor/zz/config/version.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
  MAINTENANCE=$(cat $DIR/vendor/zz/config/version.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
  TAG=$(cat $DIR/vendor/zz/config/version.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
  ...
  echo -e "${cya}Building ${bldcya}CrystalZZ $VERSION for $DEVICE ${txtrst}";
  ...
  if [ -n "${INTERACTIVE}" ]; then
          ...
          if [ "${VENDOR}" == "zz" ]; then
                  echo -e "[${bldgrn}lunch zz_$DEVICE-userdebug${bldblu}]${txtrst}"
          else
